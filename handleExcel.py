...
def getExpenses(workbook_path):
    ...
    while ws[expenseNameCol + str(categoryRow)].value is not None:
        ...
        if expenseNameCell.value in expensesDict:
            expensesDict[expenseNameCell.value][0] += float(expenseCostCell.value)
            totalCost += float(expenseCostCell.value)
        # else, add the expense name, it's cost and it's category
        else:
            expensesDict.update(
                {
                    expenseNameCell.value: [
                        float(expenseCostCell.value),
...
def fillCells(outputWorkbookPath, openAIOutput, month):
    ...
    for val in eachLineList:
        ...
        expenseNameCostCategoryDict.update(
            {
                expenseNameCostCategory[0]: [
                    expenseNameCostCategory[1],
                    expenseNameCostCategory[2],
                ]
            }
        )
    ...
    for val in expenseNameCostCategoryDict:
        ...
        while categoryRow < maxCategoryRow:
            ...
            if ws[categoryCol + str(categoryRow)].value is not None:
                if (
                    ws[categoryCol + str(categoryRow)].value
                    in expenseNameCostCategoryDict.get(val)[1]
                ):
                    # @TODO add the expense cost to the output excel file
                    if ws[expenseCol + str(categoryRow)].value is None:
                        ws[expenseCol + str(categoryRow)].value = float(
                            expenseNameCostCategoryDict.get(val)[0]
                        )
                    else:
                        ws[expenseCol + str(categoryRow)].value = float(
                            ws[expenseCol + str(categoryRow)].value
                        ) + float(expenseNameCostCategoryDict.get(val)[0])
                    ...
                    if ws[expenseCol + str(categoryRow)].comment is None:
                        ws[expenseCol + str(categoryRow)].comment = Comment(
                            "", "Automated"
                        )
                    # comment = Comment((ws[expenseCol + str(categoryRow)] ,"\n", val), "Automated")
                    comment = Comment(
                        val + " - " + str(expenseNameCostCategoryDict.get(val)[0]),
                        "Automated",
                    )
                    if comment.text != "":
                        ws[expenseCol + str(categoryRow)].comment.text += (
                            comment.text + "\n"
                        )
                    found = True
            categoryRow += 1
